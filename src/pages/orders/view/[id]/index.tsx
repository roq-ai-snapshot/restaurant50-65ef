...
function OrderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface>(
    ...
    () =>
      getOrderById(id, {
        relations: ['user_order_customer_idTouser', 'user_order_wait_staff_idTouser', 'order_item'],
      }),
  );
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Order Wait Staff Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_order_wait_staff_idTouser?.id}`}>
                      {data?.user_order_wait_staff_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
                            ...
                            <Td>
                              {hasAccess('order_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/order-items/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('order_item', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    order_itemHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
